...
Capybara.server_host =
  ENV.fetch("LOCAL_IP", IPSocket.getaddress(Socket.gethostname))
...
options.add_preference(
  ...
  default_directory: DownloadHelpers::DOWNLOAD_PATH.to_s
)
...
options.add_argument(
  "--unsafely-treat-insecure-origin-as-secure=#{Capybara.app_host}"
)
...
javascript_driver =
  case ENV.fetch("CAPYBARA_DRIVER", nil)
...
RSpec.configure do |config|
  ...
  config.before(:each, :js, type: :system) { driven_by javascript_driver }
end
